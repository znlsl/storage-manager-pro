...
interface LocalStoragePanelProps {
  currentDomain: string;
}

export const LocalStoragePanel: React.FC<LocalStoragePanelProps> = ({
  currentDomain,
}) => {
  const { t } = useLanguage();
  ...
  const loadItems = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const storageItems = await StorageService.getLocalStorage(currentDomain);
  ...
  useEffect(() => {
    const handleMessage = (message: any) => {
      if (message.type === 'STORAGE_CHANGED' && message.payload) {
        ...
        // 如果变化的Storage是localStorage且属于当前域名，刷新数据
        if (storageArea === 'localStorage' && currentDomain && domain === currentDomain) {
          console.log('LocalStorage changed for current domain, refreshing...');
          loadItems();
        }
      }
    };
  ...
  const handleDelete = async (key: string) => {
    ...
    await showConfirm(
      ...
      async () => {
        try {
          const result = await StorageService.deleteLocalStorageItem(key, currentDomain);
          if (result.success) {
            await loadItems();
          } else {
  ...
  const handleSave = async (key: string, value: string) => {
    ...
    try {
      const result = await StorageService.setLocalStorageItem(key, value, currentDomain);
      if (result.success) {
        await loadItems();
  ...
  return (
    <div className="tab-panel active">
      ...
      <StorageList
        ...
        emptyMessage={currentDomain ? t('no_localStorage_items') : t('select_domain_first')}
